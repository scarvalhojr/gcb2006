...
unmasked at step $t$, and $:=0$ otherwise.  Define $\omega(p,t):=1$ if
$p$ is masked at step $t$, and $:=0$ otherwise. Then $\sum_p\,
...
and~(\ref{eq:pos_mult}), they are not equivalent but still correlated:
a good layout has both low border lengths and low conflict indices.
...
\bibitem{CARVALHO06} de Carvalho Jr.,S., Rahmann,S. (2006)
