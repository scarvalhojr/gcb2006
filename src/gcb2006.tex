...
It now follows that, in the notation of Section~\ref{sec:model}, for a given
assignment $\pi$, we have
...
reported in percentages compared to the random layout.\label{tab:graspr_reptx_ci}}
\vspace*{2ex}
\scriptsize{
\begin{tabular}{crrcrrrcrrr}
          &            & Random & & \multicolumn{3}{c}{Row-epitaxial}  & & \multicolumn{3}{c}{GRASP with path-relinking}  \\ \cline{3-3} \cline{5-7} \cline{9-11}
...
index minimization. One reason is that it takes more time to compute matrices $F$ and
$D$ in the conflict index model as their definition are clearly more elaborate.
However, we also observed that GRASP-PR requires more time to solve the QAP with
distance matrices containing less zero entries.
